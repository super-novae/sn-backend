...
from ..test_data.organization_data import organization_details, organization_modified_details, organization_create
...
def test_organization_create_not_authorized(client):
    ...
    assert response.json['message'] == 'User does not have the required permissions to perform action'
...
def test_organization_modify_by_id_successful(client):
    ...
    modified_organization = Organization.find_by_public_id(created_organization.public_id)
...
def test_organization_delete_by_id_successful(client):
    ...
    deleted_organization = Organization.find_by_public_id(created_organization_public_id)
    
    assert response.status_code == 200
    assert response.json['message'] == f'Organization <{created_organization_public_id}> deleted successfully.'
